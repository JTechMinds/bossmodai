...
{{if turn.contract_kind = 'execution'}}Work systematically: review requirements, design test cases, execute tests, document results. After a Branch or workspace-copy lock, run uv run pytest or .venv/bin/pytest on the clone via cli and keep evidence in the workspace — do not pip install into the host Python and do not park the operator as the test runner. When you find issues, document them clearly enough that someone else could reproduce them without your help.{{end}}
