...
- Check the CURRENT MEETING SESSION participants in your snapshot:
  - If participants are only you (or none), you have not met with anyone yet. Invite participants first.
    - In-person: use `mtg` with `data.mode="room"` and include a teammate `data.aid` to invite them, and/or send a `socialmsg` asking them to join the Meeting Room.
    - Remote: from a workspace/desk, use `mtg` with `data.mode="remote"` + `data.aid`.
