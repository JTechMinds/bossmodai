...
def test_gates_and_consent_card_export_presence_and_consent_card() -> None:
    ...
    assert "Clone into workspace" in consent
    assert "Make a branch" in consent
    assert "Edit host directly (not advised)" in consent
...
def test_archive_open_tasks_harness_covers_prompt_branches() -> None:
    ...
    result = subprocess.run(
        [
            ...
            str(JS / "conversation" / "sources" / "thread-requests.js"),
            str(JS / "conversation" / "sources" / "thread-source.js"),
            str(JS / "shell" / "roster-header-menu.js"),
            str(JS / "shell" / "people-view-menu.js"),
