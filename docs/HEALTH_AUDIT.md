...
| JS `fetch('/api/...')` | `app.js`, `agent-context.js`, `settings-view.js`, `cli-policy-section.js`, `company-*.js`, `channels-view.js`, `diagnostics.js`, `activity.js` | No shared client on `main`. PR #2 adds `ui/static/js/api-auth.js` — remaining files must migrate or auth silently 401s. |
