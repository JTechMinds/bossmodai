...
A shared runtime core is injected every turn beside the Role contract: identity (name + specialty), desk/`/me`, allowed tools, host-path consent, workspace preference, checkable done, audience soft-judgment (stay quiet or one short pass when another specialty clearly fits), and chat formatting (short paragraphs, real newlines, markdown lists; no hard length limit). Role-specific quality bars stay in Description. Hire Advanced shows the core as a read-only preview. Multi-party thread messages still wake every member; this is not an intent router and does not require `@`.
